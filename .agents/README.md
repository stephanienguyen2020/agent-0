...
Package metadata lives in **`pyproject.toml`** (installable package **`agent-zero-agents`**, import **`emagents`**). Railway workers run **`pip install -e ./backend -e './agents[realtime]'`** from repo root (`[nixpacks.toml](../nixpacks.toml)`).
